...
contract DividendDistributor is IDividendDistributor {
    ...
    function changeToken(address newToken) external override onlyToken {
        ...
        require(totalDividends == 0 || previousAmount > 0, "DIVIDENDDISTRIBUTOR: Requires at least some of the initial token to calculate convertion rate.");
...
contract SmartPlacesProtocol is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string public currentlyServing = "Nothing!";

    string constant _name = "Smart Places Protocol";
    string constant _symbol = "SMARTEE";
    ...
    address public communityFundReceiver;
    ...
    constructor(address autoLiqLocker, address marketingWallet, address communityWallet) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "SPP: Recipient wallet will exceed max wallet size, blyat.");
        }
        ...
        _balances[sender] = _balances[sender].sub(amount, "SPP: Insufficient Balance, blyat.");
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "SPP: Max TX Limit Exceeded, cyka.");
    }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + anti_sniper_blocks >= block.number){ return feeDenominator.sub(1); } //This is essentially 99/100, translated to: "Make tax as close to 100% as you can".
    ...
    function setAntiSniperBlocks(uint256 blocks) public onlyOwner {
        require(blocks <= 10, "SPP: You're looking too far ahead for anti-sniping, cyka!");
    ...
    function setReflectToken(address newToken) external onlyOwner {
        require(newToken.isContract(), "SPP Reflect: Enter valid contract address");
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _communityFeeReceiver, address _marketingFeeReceiver) external onlyOwner {
    ...
    function setSwapBackSettings(bool _enabled, uint256 _denominator) external onlyOwner {
        require(_denominator > 0, "SETSWAPBACKSETTINGS: Denominator has to be >0");
